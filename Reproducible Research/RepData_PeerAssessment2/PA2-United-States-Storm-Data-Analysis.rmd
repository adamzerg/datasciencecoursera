---
title: "United States Storm Date Analysis 1950 - 2011"
date: "`r Sys.Date()`"
output: 
  html_document:
    keep_md: true
...
Preload of packages in used, mainly from [Tidyverse](https://www.tidyverse.org/packages/) and [Plotly ggplot2](https://plotly.com/ggplot2/)
...
The data for this project come in the form of a comma-separated-value file compressed via the bzip2 algorithm to reduce its size. Aside of this we added event tibble for better mapping of all the events appears in the data, and states abbreviations for mapping of the geo locations.
...
2. Event table contains of 48 event types, pulled from the [Documentation](https://d396qusza40orc.cloudfront.net/repdata%2Fpeer2_doc%2Fpd01016005curr.pdf)
...
```{r ingest-dataset, message=FALSE}
...
When profiling data, we immediataly found that EVTYPE is the descriptive keword but there are 977 distinct values. Most of them are dirty due to several reasons:
...
4. More than one event may be listed togather e.g. "DROUGHT/EXCESSIVE HEAT", "HIGH WINDS/FLOODING"
...
- The unwanted row and words are cleared away
- Use precised keword to correct event name
...
- Correct most event with starting keywords
    As if two event appears we consider the one appear first
- Take extra care for ending keywords
    We meant not to overlapped the result being corrected ready by starting keywords
- Assumptions are made here according to the 48 type of events listed in the documentation
...
- 977 EVTYPE is cleaned to 317 Events
...
```{r plot-pop-treemap, message=FALSE}
p2s <- df %>%
...
```{r plot-dmg-treemap, message=FALSE}
p4s <- df %>%
    group_by(Designator, Event) %>%
    summarise(
            PropDmgAmt.Sum = sum(PropDmgAmt, na.rm = TRUE),
